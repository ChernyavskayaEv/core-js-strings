...
function removeFirstOccurrences(str, value) {
  const strBeforeVaLue = str.slice(0, str.indexOf(value));
  const strAfterVaLue = str.slice(str.indexOf(value) + value.length);

  return str.includes(value) ? `${strBeforeVaLue}${strAfterVaLue}` : str;
}
...
function removeLastOccurrences(str, value) {
  const strBeforeVaLue = str.slice(0, str.lastIndexOf(value));
  const strAfterVaLue = str.slice(str.lastIndexOf(value) + value.length);
...
function sumOfCodes(str) {
  return !str || str.length === 0
    ? 0
    : [...str].reduce((acc, letter) => acc + Number(letter.charCodeAt(0)), 0);
}
...
function countVowels(str) {
  ...
  return [...str].reduce(
    (acc, letter) => (!vowels.includes(letter) ? acc : acc + 1),
    0
  );
}
...
function isPalindrome(str) {
  const re = /[?!;:,. ]/gi;
  const checkStr = str.replace(re, '');
  for (let i = 0; i <= Math.floor(checkStr.length / 2); i += 1) {
    if (
      checkStr[i].toLowerCase() !==
      checkStr[checkStr.length - 1 - i].toLowerCase()
    )
      return false;
  }
  return true;
...
function invertCase(str) {
  return [...str]
    .map((item) => {
      ...
      if (charCode > 64 && charCode < 91) return item.toLowerCase();
      if (charCode > 96 && charCode < 123) return item.toUpperCase();
      return item;
    })
    .join('');
}
...
function encodeToRot13(str) {
  return [...str]
    .map((item) => {
      if (!'?! '.includes(item)) {
        const modifiedCharCode =
          (item.charCodeAt(0) > 77 && item.charCodeAt(0) < 91) ||
          (item.charCodeAt(0) > 109 && item.charCodeAt(0) < 123)
            ? item.charCodeAt(0) - 13
            : item.charCodeAt(0) + 13;
        return String.fromCharCode(modifiedCharCode);
      }
      return item;
    })
    .join('');
...
function getCardId(value) {
  ...
  return setOfCards.indexOf(value);
}
